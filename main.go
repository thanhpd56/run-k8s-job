...
func main() {
	...
	input := ActionInput{
		...
		dsn:             action.GetInput("dsn"),
		migrationSource: action.GetInput("migration-source"),
	}
	...
	runner := NewJobRunner(jobs, pods, 5*time.Second, action, namespace, *clientset, database{
		dsn:             input.dsn,
